 ...
 * @since 7.4
 */
final class SnomedComplexBlockMapMemberUpdateDelegate extends SnomedRefSetMemberUpdateDelegate {

	SnomedComplexBlockMapMemberUpdateDelegate(SnomedRefSetMemberUpdateRequest request) {
		super(request);
	}
	...
	boolean execute(SnomedRefSetMemberIndexEntry original, SnomedRefSetMemberIndexEntry.Builder member, TransactionContext context) {
		...
		Integer newMapBlock = getProperty(SnomedRf2Headers.FIELD_MAP_BLOCK, Integer.class);
		...
		if (newMapBlock != null && newMapBlock.intValue() != original.getMapBlock()) {
			member.field(SnomedRf2Headers.FIELD_MAP_BLOCK, newMapBlock);
			changed |= true;
		}
