...
//		final SnomedDescriptionIndexEntry fsn = fsn(conceptId, ImmutableMap.of(
//			Concepts.REFSET_LANGUAGE_TYPE_UK, Acceptability.PREFERRED,
//			Concepts.REFSET_LANGUAGE_TYPE_US, Acceptability.ACCEPTABLE
//		));
//		final SnomedConceptDocument concept = concept(conceptId)
//				.preferredDescriptions(ImmutableList.of(
//					new SnomedDescriptionFragment(fsn.getId(), fsn.getTypeId(), fsn.getTerm(), Concepts.REFSET_LANGUAGE_TYPE_UK)
//				))
//				.build();
...
//		
//		initRevisions(concept, fsn, preferredInUk);
//		
...
//		final String conceptId = generateConceptId();
//		final SnomedDescriptionIndexEntry description = fsn(conceptId, ImmutableMap.of(Concepts.REFSET_LANGUAGE_TYPE_UK, Acceptability.PREFERRED, Concepts.REFSET_LANGUAGE_TYPE_US, Acceptability.PREFERRED));
//		final SnomedConceptDocument concept = concept(conceptId)
//				.preferredDescriptions(ImmutableList.of(
//					new SnomedDescriptionFragment(description.getId(), Concepts.FULLY_SPECIFIED_NAME, description.getTerm(), ImmutableList.of(Concepts.REFSET_LANGUAGE_TYPE_UK, Concepts.REFSET_LANGUAGE_TYPE_US))
//				))
//				.build();
//		SnomedRefSetMemberIndexEntry memberToInactivate = langMember(description.getId(), Acceptability.PREFERRED, Concepts.REFSET_LANGUAGE_TYPE_US);
//		
//		indexRevision(MAIN, concept, description, memberToInactivate);
//
//		stageChange(memberToInactivate, SnomedRefSetMemberIndexEntry.builder(memberToInactivate).active(false).build());
//		
//		final ConceptChangeProcessor processor = process();
//		
//		final SnomedConceptDocument expected = docWithDefaults(concept)
//				.preferredDescriptions(ImmutableList.of(
//					new SnomedDescriptionFragment(description.getId(), Concepts.FULLY_SPECIFIED_NAME, description.getTerm(), Concepts.REFSET_LANGUAGE_TYPE_UK)
//				))
//				.build();
//		final Revision actual = Iterables.getOnlyElement(processor.getChangedMappings().values()).getNewRevision();
//		assertDocEquals(expected, actual);
...
//				))
//				.build();
//		SnomedRefSetMemberIndexEntry memberToChange = langMember(description.getId(), Acceptability.PREFERRED, Concepts.REFSET_LANGUAGE_TYPE_US);
//		
//		indexRevision(MAIN, concept, description, memberToChange);
//		
//		stageChange(memberToChange, SnomedRefSetMemberIndexEntry.builder(memberToChange).acceptabilityId(Concepts.REFSET_DESCRIPTION_ACCEPTABILITY_ACCEPTABLE).build());
//		
//		final ConceptChangeProcessor processor = process();
//		
//		final SnomedConceptDocument expected = docWithDefaults(concept)
